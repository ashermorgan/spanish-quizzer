...
function Start() {
    ...
    var filter = document.getElementById(`settingsSetFilter`).value;
    Terms = ApplyFilter(Verbs, filter);
    ...
    localStorage.setItem("verb-terms", JSON.stringify(Terms));
...
function Resume() {
    ...
    if (!confirm("This will remove the filter you have already selected. Are you sure?")) {
        return;
    }
    ...
    Terms = JSON.parse(localStorage.getItem("verb-terms"));
    Term = parseInt(localStorage.getItem("verb-term")) - 1;
...
function Reset() {
    ...
    localStorage.setItem("verb-term", Term);
